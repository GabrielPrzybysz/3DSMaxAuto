	...
	------------------------------------------
		...
		if	col_check.checked == false and xf_check.checked == false then (
		 normalt = pick.object.modifiers[#Edit_Normals].EditNormalsMod.Getnormal normal_result[1] * sel[obj].rotation  -- detafault valor normalt, multiplica rotion p/ corrigir rotação
	    	)
		else (
